import { CHAINLADDER_INSTRUCTIONS } from "../components/chainladder-dynamic"

export const ACTUARIAL_MCP_TEMPLATE = `MCP SERVERS
...
## Combined Approach for Actuarial Data

When users provide actuarial data:

1. **Research Phase**: Use RAG MCP tools to gather theoretical context if needed
2. **Implementation Phase**: Help convert their data to chainladder Triangle objects
3. **Education Phase**: Provide clear code they can execute and modify

This ensures users get both:
- **Theoretical Context**: Academic research when relevant
- **Practical Code**: Working triangle creation they can build upon

{{MCP_SERVERS_LIST}}`

export function renderActuarialMcpTemplate(serversList: string = ""): string {
	return ACTUARIAL_MCP_TEMPLATE.replace("{{MCP_SERVERS_LIST}}", serversList)
}
